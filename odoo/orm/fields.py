...
class Field(typing.Generic[T]):
    ...
    def _get_cache(self, env: Environment) -> MutableMapping[IdType, typing.Any]:
        ...
        try:
            ...
            #field_cache = {str(k) if isinstance(k, uuid.UUID) else k: v for k, v in field_cache.items()}
    ...
    def _insert_cache(self, records: BaseModel, values: Iterable) -> None:
        ...
        # if records._name == 'res.company' and self.name == 'active':
        #     for rid, val in zip(records._ids, values):
        #         _logger.warning(
        #             "[UUID DEBUG] _insert_cache model=%s field=%s id=%s existing=%r new=%r",
        #             records._name, self.name, rid, field_cache.get(rid, None), val
        #         )
        collections.deque(map(field_cache.setdefault, records._ids, values), maxlen=0)
        # if records._name == 'res.company' and self.name == 'active':
        #     for rec_id, val in zip(records._ids, values):
        #         cur = field_cache.get(rec_id, '<missing>')
        #         _logger.warning(
        #             "[UUID DEBUG] [_insert_cache:verify] model=%s field=%s id=%s wrote=%r now_in_cache=%r",
        #             records._name, self.name, rec_id, val, cur
        #         )
    ...
    #                 _logger.warning("[UUID DEBUG] Fetching field '%s' for model=%s, ids=%s",
    #                         self.name, record._name, recs._ids)
    ...
    #         if record._name == 'res.company':
    #             _logger.warning("[UUID DEBUG] After fetch: cache[%s]=%s", record_id, value)
    #         if value is SENTINEL:
    #             if record._name == 'res.company':
    #                 _logger.error("[UUID DEBUG] Cache miss for %s(%s). Field=%s Store=%s - record.exists()=%s",
    #                     record._name, record_id, self.name, self.store, record.exists())
    #             raise MissingError("\n".join([
    ...
    def __get__(self, record: BaseModel, owner=None) -> T:
        """Return the value of field ``self`` on ``record`` with debug for res.company."""
        if record is None:
            return self
        ...
        # --- DEBUG: log record id and cache keys ---
        # if record._name == 'res.company':
        #     _logger.warning("[UUID DEBUG] __get__ start: model=%s, field=%s, record_id=%s, type=%s",
        #                     record._name, self.name, record_id, type(record_id))
        #     _logger.warning("[UUID DEBUG] __get__: cache_keys before read=%s", list(field_cache.keys()))
        ...
        # --- fetch or prefetch if stored ---
        if self.store and record_id:
            recs = self._to_prefetch(record)
            try:
                # if record._name == 'res.company':
                #     _logger.warning("[UUID DEBUG] Fetching field '%s' for model=%s, prefetch_ids=%s",
                #                     self.name, record._name, recs._ids)
                recs._fetch_field(self)
            except AccessError:
                if len(recs) == 1:
            ...
            # if record._name == 'res.company':
            #     _logger.warning("[UUID DEBUG] After fetch: model=%s, field=%s, record_id=%s, value=%r, cache_keys=%s",
            #                     record._name, self.name, record_id, value, list(field_cache.keys()))

            if value is SENTINEL:
                # if record._name == 'res.company':
                #     _logger.error("[UUID DEBUG] Cache miss! model=%s, field=%s, record_id=%s, _ids=%s, cache_keys=%s",
                #                 record._name, self.name, record_id, record._ids, list(field_cache.keys()))
                raise MissingError("\n".join([
                    env._("Record does not exist or has been deleted."),
                    env._("(Record: %(record)s, User: %(user)s)", record=record, user=env.uid),
                ])) from None
