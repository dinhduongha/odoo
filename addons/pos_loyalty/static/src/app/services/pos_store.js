...
// New (unsaved) loyalty cards are identified by a negative int sentinel
// generated by loyaltyIdsGenerator. Persisted cards have a uuid (string) id.
export function isNewCouponId(couponId) {
    return typeof couponId === "number" && couponId < 0;
}

export function isExistingCouponId(couponId) {
    return couponId && !isNewCouponId(couponId);
}
...
patch(PosStore.prototype, {
    ...
    computePartnerCouponIds(loyaltyCards = null) {
        ...
        for (const card of cards) {
            if (!card.partner_id || isNewCouponId(card.id) || !card.program_id) {
                continue;
            }
        ...
        for (const order of orders) {
            Object.assign(
                ...
                order.lines.reduce((agg, line) => {
                    if (line.coupon_id && isNewCouponId(line.coupon_id.id)) {
