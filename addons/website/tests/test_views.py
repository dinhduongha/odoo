...
class TestViewSaving(TestViewSavingCommon):
    ...
    def setUp(self):
        ...
        self.arch = h.DIV(
            ...
            h.DIV(
                ...
                h.UL(
                    h.LI("Item 1"),
                    h.LI(h.SPAN("My Company", attrs(model='res.company', id=self.company_id, field='name', type='char'))),
                    h.LI(h.SPAN("+00 00 000 00 0 000", attrs(model='res.company', id=self.company_id, field='phone', type='char')))
                ))
        )
    ...
    def test_embedded_extraction(self):
        ...
        expect = [
            h.SPAN("My Company", attrs(model='res.company', id=self.company_id, field='name', type='char')),
            h.SPAN("+00 00 000 00 0 000", attrs(model='res.company', id=self.company_id, field='phone', type='char')),
        ]
    ...
    def test_embedded_save(self):
        embedded = h.SPAN("+00 00 000 00 0 000", attrs(
            model='res.company', id=self.company_id, field='phone', type='char'))
    ...
    def test_embedded_conflict(self):
        e1 = h.SPAN("My Company", attrs(model='res.company', id=self.company_id, field='name'))
        e2 = h.SPAN("Leeroy Jenkins", attrs(model='res.company', id=self.company_id, field='name'))
    ...
    def test_to_field_ref_keep_attributes(self):
        ...
        att = attrs(expression="bob", model="res.company", id=self.company_id, field="name")
        att['id'] = "whop"
        att['class'] = "foo bar"
    ...
    def test_save(self):
        ...
        replacement = ET.tostring(h.DIV(
            ...
            h.UL(
                h.LI("wob wob wob"),
                h.LI(h.SPAN("Acme Corporation", attrs(model='res.company', id=self.company_id, field='name', expression="bob", type='char'))),
                h.LI(h.SPAN("+12 3456789", attrs(model='res.company', id=self.company_id, field='phone', expression="edmund", type='char'))),
            )
        ), encoding='unicode')

        self.view_id.with_context(website_id=self.env.ref('website.default_website').id).save(value=replacement, xpath='/div/div[2]')
        ...
        self.env['website'].with_context(website_id=self.env.ref('website.default_website').id).viewref(self.view_id.key).unlink()
        ...
        company = Company.browse(self.company_id)
    ...
    def test_save_oe_structure_with_attr(self):
        ...
        view = self.env['ir.ui.view'].create({
            ...
            'type': 'qweb'
        }).with_context(website_id=self.env.ref('website.default_website').id, load_all_views=True)
...
class TestCowViewSaving(TestViewSavingCommon, HttpCase):
    ...
    def test_cow_on_base_after_extension(self):
        View = self.env['ir.ui.view']
        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Extension Specific'})
        v1 = self.base_view
        v2 = self.inherit_view
        v3 = View.search([('website_id', '=', self.env.ref('website.default_website').id), ('name', '=', 'Extension Specific')])
        v4 = self.inherit_view.copy({'name': 'Second Extension'})
        v5 = self.inherit_view.copy({'name': 'Third Extension (Specific)'})
        v5.write({'website_id': self.env.ref('website.default_website').id})
        ...
        v1.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Base Specific'})
        ...
        v6 = View.search([('website_id', '=', self.env.ref('website.default_website').id), ('name', '=', 'Base Specific')])
        v7 = View.search([('website_id', '=', self.env.ref('website.default_website').id), ('name', '=', 'Extension Specific')])
        v8 = View.search([('website_id', '=', self.env.ref('website.default_website').id), ('name', '=', 'Second Extension')])
        v9 = View.search([('website_id', '=', self.env.ref('website.default_website').id), ('name', '=', 'Third Extension (Specific)')])
    ...
    def test_cow_leaf(self):
        ...
        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div position="replace"><div>website 1 content</div></div>'})
        inherit_views = View.search([('key', '=', 'website.extension_view')])
        self.assertEqual(View.search_count([('key', '=', 'website.base_view')]), 1)
        self.assertEqual(len(inherit_views), 2)
        self.assertEqual(len(inherit_views.filtered(lambda v: v.website_id.id == self.env.ref('website.default_website').id)), 1)

        # read in backend should be unaffected
        arch = self.base_view.get_combined_arch()
        self.assertEqual(arch, '<div>modified content</div>')
        # read on website should reflect change
        arch = self.base_view.with_context(website_id=self.env.ref('website.default_website').id).get_combined_arch()
        ...
        inherit_views.filtered(lambda v: v.website_id.id == self.env.ref('website.default_website').id).write({'active': False})
        arch = self.base_view.with_context(website_id=self.env.ref('website.default_website').id).get_combined_arch()
    ...
    def test_cow_root(self):
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div>website 1 content</div>'})

        generic_base_view = View.search([('key', '=', 'website.base_view'), ('website_id', '=', False)])
        website_specific_base_view = View.search([('key', '=', 'website.base_view'), ('website_id', '=', self.env.ref('website.default_website').id)])
        ...
        arch = website_specific_base_view.with_context(load_all_views=True, website_id=self.env.ref('website.default_website').id).get_combined_arch()
    ...
    #
    #     # edit on frontend, copy just the leaf
    #     self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div position="replace"><div>website 1 content</div></div>'})
    ...
    def test_cow_generic_view_with_already_existing_specific(self):
        ...
        base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'New Name'})  # This will not write on `base_view` but will copy it to a specific view on which the `name` change will be applied
        specific_view = View.search([['name', '=', 'New Name'], ['website_id', '=', 1]])
        base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Another New Name'})
        specific_view.active = False
        base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Yet Another New Name'})
        ...
        total_views = View.with_context(active_test=False).search_count([])
        second_view.with_context(website_id=self.env.ref('website.default_website').id).save('<div class="editable_part" data-oe-id="%s" data-oe-xpath="/t[1]/t[1]/div[1]/div[1]" data-oe-field="arch" data-oe-model="ir.ui.view">First editable_part</div>' % second_view.id, "/t[1]/t[1]/div[1]/div[1]")
        second_view.with_context(website_id=self.env.ref('website.default_website').id).save('<div class="editable_part" data-oe-id="%s" data-oe-xpath="/t[1]/t[1]/div[1]/div[3]" data-oe-field="arch" data-oe-model="ir.ui.view">Second editable_part</div>' % second_view.id, "/t[1]/t[1]/div[1]/div[3]")
    ...
    def test_cow_complete_flow(self):
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div>Bye</div>'})
        ...
        base_specific = View.search([('key', '=', self.base_view.key), ('website_id', '=', self.env.ref('website.default_website').id)]).with_context(load_all_views=True)
        extend_specific = View.search([('key', '=', self.inherit_view.key), ('website_id', '=', self.env.ref('website.default_website').id)])
        ...
        arch = base_specific.with_context(website_id=self.env.ref('website.default_website').id).get_combined_arch()
        self.assertEqual('Bye All' in arch, True)

        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div position="inside"> Nobody</div>'})
        ...
        base_arch_w1 = self.base_view.with_context(website_id=self.env.ref('website.default_website').id).get_combined_arch()
    ...
    def test_cow_cross_inherit(self):
        ...
        child_view_2.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<xpath expr="//p" position="replace"><span>D</span></xpath>'})
        ...
        main_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<body>SPECIFIC<div>Z</div></body>'})
        ...
        specific_view = View.with_context(website_id=self.env.ref('website.default_website').id)._get_template_view("website.main_view")
        generic_view_arch = generic_view.with_context(load_all_views=True).get_combined_arch()
        specific_view_arch = specific_view.with_context(load_all_views=True, website_id=self.env.ref('website.default_website').id).get_combined_arch()
    ...
    def test_multi_website_view_active(self):
        ...
        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'active': False})
        ...
        inherit_view = View.with_context(website_id=self.env.ref('website.default_website').id)._get_template_view(self.inherit_view.key)
        ...
        views = View.with_context(website_id=self.env.ref('website.default_website').id).get_related_views(self.base_view.key)
        ...
        view = View.with_context(active_test=False, website_id=self.env.ref('website.default_website').id).search([('key', '=', self.inherit_view.key)]).filter_duplicate()
    ...
    def test_get_related_views_tree(self):
        ...
        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Extension'})  # Trigger cow on hierarchy
        View.create({
            'name': 'II2',
            'mode': 'extension',
            'inherit_id': self.inherit_view.id,
        ...
        views = View.with_context(website_id=self.env.ref('website.default_website').id).get_related_views('B')
    ...
    def test_get_related_views_tree_recursive_t_call_and_inherit_inactive(self):
        ...
        views = View.with_context(website_id=self.env.ref('website.default_website').id).get_related_views('_website_sale.products')
        self.assertEqual(views, products + products_item + add_to_wishlist + products_list_view, "The four views should be returned.")
        add_to_wishlist.with_context(website_id=self.env.ref('website.default_website').id).write({'active': False})  # Trigger cow on hierarchy
        add_to_wishlist_cow = Website.with_context(website_id=self.env.ref('website.default_website').id).viewref(add_to_wishlist.key)
        views = View.with_context(website_id=self.env.ref('website.default_website').id).get_related_views('_website_sale.products')
    ...
    def test_cow_inherit_children_order(self):
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Product (W1)'})
    ...
    def test_write_order_vs_cow_inherit_children_order(self):
        ...
        """
        View = self.env['ir.ui.view']
        self.inherit_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Specific Inherited View Changed First'})
        specific_view = View.search([('name', '=', 'Specific Inherited View Changed First')])
        views = View.browse([self.base_view.id, specific_view.id])
        views.with_context(website_id=self.env.ref('website.default_website').id).write({'active': False})
    ...
    def test_module_new_inherit_view_on_parent_already_forked(self):
        ...
        specific_view_arch = Website.with_context(load_all_views=True, website_id=self.env.ref('website.default_website').id).viewref('_website_sale.product').get_combined_arch()
    ...
    def test_module_new_inherit_view_on_parent_already_forked_xpath_replace(self):
        ...
        # Trigger cow on website_sale hierarchy for website 1
        base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Main Frontend Layout (W1)'})
    ...
    def test_multiple_inherit_level(self):
        ...
        self.inherit_view.website_id = self.env.ref('website.default_website').id
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div>modified content</div>'})
        ...
        base_specific = View.search([('key', '=', self.base_view.key), ('website_id', '=', self.env.ref('website.default_website').id)]).with_context(load_all_views=True)
        extend_specific = View.search([('key', '=', 'website.extension_view'), ('website_id', '=', self.env.ref('website.default_website').id)])
    ...
    def test_cow_extension_with_install(self):
        ...
        v1.with_context(website_id=self.env.ref('website.default_website').id).write({'name': 'Extension Specific'})
    ...
    def test_specific_view_translation(self):
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'active': True})
    ...
    def test_view_translation_without_website(self):
        ...
        self.env.ref('website.default_website').default_lang_id = fr_BE
        self.base_view.with_context(lang='en_US').arch_db = '<div>hello</div>'
        self.base_view.update_field_translations('arch_db', {'fr_BE': {'hello': 'bonjour'}})
    ...
    def test_load_module_terms_preserve_delayed_translation(self):
        ...
        base_footer.with_context(website_id=self.env.ref('website.default_website').id).write({'active': True})
    ...
    def test_specific_view_module_update_inherit_change(self):
        ...
        self.base_view.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div>website 1 content</div>'})
        specific_view = Website.with_context(load_all_views=True, website_id=self.env.ref('website.default_website').id).viewref(self.base_view.key)
        specific_view.inherit_children_ids.with_context(website_id=self.env.ref('website.default_website').id).write({'arch': '<div position="inside">, extended content website 1</div>'})
        specific_child_view = Website.with_context(load_all_views=True, website_id=self.env.ref('website.default_website').id).viewref(self.inherit_view.key)
    ...
    def test_no_cow_on_translate(self):
        ...
        View = self.env['ir.ui.view'].with_context(lang=french.code, website_id=self.env.ref('website.default_website').id)
        old_specific_views = View.search([('website_id', '!=', None)])
        view = self.base_view.with_context(lang=french.code, website_id=self.env.ref('website.default_website').id)
