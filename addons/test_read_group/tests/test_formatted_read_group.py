...
class TestFormattedReadGroup(common.TransactionCase):
    ...
    def test_groupby_chain_fnames_many2one(self):
        ...
        alias_join = make_identifier(f"test_read_group_related_base__foo_id__{self.base_user.id}")
    ...
    def test_groupby_chain_fnames_char(self):
        ...
        # Same query generated by grouping foo_id.bar_id.name/foo_id.bar_name_sudo
        alias_join = make_identifier(f"test_read_group_related_base__foo_id__{self.base_user.id}")
        alias_bar = make_identifier(f"{alias_join}__bar_id")
        expected_query = f"""
            SELECT "{alias_bar}"."name",
                   COUNT(*)
            FROM "test_read_group_related_base"
            LEFT JOIN (
                SELECT "test_read_group_related_foo".*
            ...
            LEFT JOIN "test_read_group_related_bar" AS "{alias_bar}" ON (
                "{alias_join}"."bar_id" = "{alias_bar}"."id"
            )
            GROUP BY "{alias_bar}"."name"
            ORDER BY "{alias_bar}"."name" ASC
        """
...
class TestFormattedReadGroupMonetary(common.TransactionCase):
    ...
    def test_sum_monetary_rated_eur_company(self):
        # Use a fresh company so that changing its currency is not blocked by
        # pre-existing journal items (e.g. account demo data in the database).
        company = self.env['res.company'].create({'name': 'EUR Company'})
        ...
        MonetaryAgg = self.MonetaryAgg.with_env(self.env)
        self.env['res.currency.rate'].create([
            {
        ...
        MonetaryAgg.formatted_read_group([], [], aggregates)
        with self.assertQueries(["""
            SELECT
                SUM("test_read_group_aggregate_monetary"."total_in_currency_id"),
                ARRAY_AGG(
