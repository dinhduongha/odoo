...
class AccountMove(models.Model):
    ...
    def _get_invoice_reference_be_partner(self):
        ...
        """
        self.ensure_one()
        partner_id_digits = re.sub(r'\D', '', str(self.partner_id.id))
        bbacomm = (re.sub(r'\D', '', self.partner_id.ref or '') or partner_id_digits)[-10:].rjust(10, '0')
        base = int(bbacomm)
        mod = base % 97 or 97
    ...
    def _get_invoice_reference_be_invoice(self):
        ...
        self.ensure_one()
        base = self.id
        bbacomm = re.sub(r'\D', '', str(base))[-10:].rjust(10, '0')
        base = int(bbacomm)
        mod = base % 97 or 97
        reference = '+++%s/%s/%s%02d+++' % (bbacomm[:3], bbacomm[3:7], bbacomm[7:], mod)
        return reference
