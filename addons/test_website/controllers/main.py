...
class WebsiteTest(Home):
    ...
    def test_company_context(self):
        return request.make_response(json.dumps(request.env.context.get('allowed_company_ids'), default=str))
    ...
    @http.route(['/test_website/model_item/<string:record_id>'], type='http', methods=['GET'], auth="public", website=True, sitemap=False)
    ...
    @http.route(['/test_website/model_item_sudo/<string:record_id>'], type='http', methods=['GET'], auth="public", website=True, sitemap=False)
